...
LearnerSurvsjive = R6Class("LearnerSurvsjive",
  ...
  private = list(
    .train = function(task) {
      ...
      fit_jive = do.call(sup.r.jive::sJIVE,
                         c(list(X=lapply(block_list, as.matrix), Y=resi),
                           jive_args))

      pred = predict(newdata=lapply(block_list, as.matrix),
        fit_jive)
      ...
      if(pars$rankJ > 0) {
        latent_space[,1:pars$rankJ] = t(pred$Sj)
      }
 ...
 #     )
      ...
      for (f in folds) {
        ...
        jive_train = do.call(sup.r.jive::sJIVE, c(list(X=fold_train, Y=resi),
                             jive_args))

        train_pred = predict(newdata=fold_train, jive_train)
        ...
        if(pars$rankJ > 0) {
          latent_space_train[,1:pars$rankJ] = t(train_pred$Sj)
        }
        
        if(pars$rankA > 0) {
          latent_space_train[,(pars$rankJ+1):ncol(latent_space_train)] = 
            t(Reduce(rbind, train_pred$Si)) # cbind was not working
        }
        ...
        jive_test = predict(newdata=fold_test, jive_train)

        latent_space_test = matrix(0,
          nrow=length(f), ncol=pars$rankJ+length(block_list)*pars$rankA)

        if(pars$rankJ > 0) {
          latent_space_test[,1:pars$rankJ] = t(jive_test$Sj)
        }
        
        if(pars$rankA > 0) {
          latent_space_test[,(pars$rankJ+1):ncol(latent_space_test)] = 
            t(Reduce(rbind, jive_test$Si)) # cbind was not working
        }
